...
## 4. register —— 注册到 A2A registry

注册任意已部署 Runtime Agent 时，使用顶层命令，不需要当前项目的 Harness 名称：
...
| 参数 | 说明 | 默认值 |
| :--- | :--- | :--- |
| `--space-id` / `--register-space-id` | A2A SpaceId。 | — |
| `--runtime-id` / `--register-runtime-id` | 要注册的 RuntimeId（仅顶层 `agentkit register` 必填）。 | — |
| `--network-type` / `--register-network-type` | Runtime 网络地址类型。 | `public` |
| `--endpoint` / `--register-endpoint` | AgentKit OpenAPI endpoint。 | 按 region 推导 |
| `--region` / `--register-region` | AgentKit OpenAPI region。 | `AGENTKIT_REGION` / `VOLCENGINE_REGION` / `cn-beijing` |
| `--tag` / `--register-tag` | `KEY=VALUE` 标签，可重复。 | — |
