...
`agentkit register` 用于把任意已部署的 Runtime Agent 注册到 A2A registry。它不依赖当前项目的 harness 名称，只需要显式传入 A2A SpaceId 和 RuntimeId。

### 命令格式

```bash
agentkit register --space-id <A2A_SPACE_ID> --runtime-id <RUNTIME_ID> [options]
```
...
| 参数 | 说明 | 示例 |
| :--- | :--- | :--- |
| `--space-id`, `--register-space-id` | **（必填）** A2A SpaceId。 | `--space-id as-xxx` |
| `--runtime-id`, `--register-runtime-id` | **（必填）** 要注册的 RuntimeId。 | `--runtime-id r-xxx` |
| `--network-type`, `--register-network-type` | Runtime 网络地址类型。 | `--network-type public` |
| `--endpoint`, `--register-endpoint` | AgentKit OpenAPI endpoint。 | `--endpoint https://agentkit.cn-beijing.volcengineapi.com/` |
| `--region`, `--register-region` | AgentKit OpenAPI region。 | `--region cn-beijing` |
| `--tag`, `--register-tag` | `KEY=VALUE` 标签，可重复。 | `--tag env=prod` |
